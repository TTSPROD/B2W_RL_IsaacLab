...
from run_contact6_ablation import compare,training_spec
...
def results():
    ...
    return {a:{p:copy.deepcopy(good) for p in ('nominal','bounded_v1')} for a in ('reference','seed50_control','seed50_contact6','seed51_control','seed51_contact6')}

class Contact6ComparisonTests(unittest.TestCase):
    ...
    def test_variant_requires_every_seed_profile_and_tracking(self):
        ...
        self.assertEqual(compare(r)['selected_for_future_replication'],'contact6')
        r['seed51_contact6']['bounded_v1']['by_scenario']['yaw_positive']['pooled_rms_vx_vy_yaw'][2]=.26
    ...
    def test_wrong_parent_seed_or_boundary_rejected(self):
        for parent in ({'seed':49,'ending_runner_iteration':2499},{'seed':50,'ending_runner_iteration':3999}):
            with self.assertRaises(ValueError):training_spec(50,'contact6',parent)
    ...
    def test_contact_override_keeps_default_and_rejects_positive_or_nan(self):
        ...
        with patch.object(sys,'argv',['train_b2w_desktop','--undesired_contact_weight','-3']):
            self.assertEqual(parse_args(Launcher).undesired_contact_weight,-3.)
        for value in ('0','1','nan','inf'):
            with patch.object(sys,'argv',['train_b2w_desktop','--undesired_contact_weight',value]), patch('sys.stderr'):
