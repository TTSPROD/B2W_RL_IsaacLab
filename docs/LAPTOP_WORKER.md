...
**Основное обучение ноутбуку не назначено.** Пользователь выбрал пока только
проверить сервер и оставить desktop queue прежней. Seeds 49/50/51 и все восемь
оценок остаются на исходном ПК по [протоколу](STAGED_QUALIFICATION.md).
Качество ноутбучной policy не оценивалось; benchmark не является принятой политикой.
...
- Поверх него переданы текущие scripts/docs/requirements с покомпонентными
  SHA256 и резервными копиями заменённых файлов в `.cache/laptop-sync/before`.
  Исходники активных desktop trainers не изменяются. `vendor/` не переписывается.
...
## Инструменты квалификации и подготовленные варианты передачи
...
P-ядрами через `laptop_pcore_runtime.py`. Последний статус worker —
...
исходным координатором. Подготовленные launchers нельзя запускать как способ
продолжить нынешнее состояние: потребуется новое назначение и проверка условий.
