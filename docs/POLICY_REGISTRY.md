...
## Область приемки с 25 сентября

Принимается низкоуровневая policy по внешним `(vx, vy, omega_z)`, ABI57→16.
Development `locomotion57_v1` реализован: upstream10000/15000/19999 проверены в
5184 эпизодах Isaac/MuJoCo. Их новый статус — **rejected in development screen**;
квалифицированного checkpoint нет. Остальные кандидаты пока **not evaluated**
по этому протоколу. [Результаты и ограничения](results/2026-09-25-upstream-locomotion57.md). Историческая
Flat54 qualification остается действительной только в собственном Flat scope.
ABI60 остается несовместимой отклонённой абляцией.

Последующий contact57 у19999:17/20 diagnostic success, unsafe0; статус остается
`not accepted`. **Upstream10000 больше не запускать и не исследовать по указанию
пользователя.** Его строки и SHA сохраняются как история. Новый upstream feedback
выполняется с19999. [Отчет](results/2026-09-25-contact57-diagnostics.md).

Последующий cooked-geometry screen19999:12/20, unsafe0; прежние17/20 относятся к
...
## Историческое решение и сохраняемые ограничения

- Принятой общей политики для Rough/Stairs и реального робота **нет**.
- Исследовательский parent прежней постановки — серверный `inverse57 updates03000`:
...
**working parent** — выбранный источник прежнего эксперимента, не релиз;
**experimental** — обучен, но не квалифицирован; **rejected** — проверен и отклонён
по указанному историческому протоколу; **not evaluated** — соответствующий протокол
еще не выполнен. Статус всегда сопровождается областью и версией приемки; отсутствие
новой оценки не отменяет старый fail и не разрешает продолжать закрытые runs.
...
Для новой приемки обязательны gates `locomotion57_v1` из
[PROJECT_PLAN.md](PROJECT_PLAN.md#acceptance-gates-низкоуровневая-locomotion-policy):
tracking внешних команд, смена/обнуление команды, непрерывное устойчивое стояние,
проходимость, safety/actuation, независимая validation и тот же export в обоих
симуляторах. Полный навигационный цикл, corridor и landing-stop не оцениваются.
Результат записывается отдельно от исторического статуса: protocol/config/evaluator
hash, checkpoint/export SHA, terrain/command envelope, seeds, фактические метрики
и решение. Численные пороги не дублируются в реестре.

Интерактивный viewer — ручная диагностика. После симуляционной приемки отдельно
выполняются offline SDK2 replay/dry-run и стендовые hardware gates с явным допуском.
До них статус реального деплоя остается `not authorized`.
