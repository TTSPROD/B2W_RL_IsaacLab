...
| [Nahrendra et al., DreamWaQ, 2023](https://arxiv.org/abs/2301.10602) | Temporal proprioception (`H=5`), privileged critic и context/velocity estimator позволяют blind adaptation | Для stair-v4 сравнить exact-state teacher с deployable history student; скорость не добавлять как один изолированный столбец без target/phase |
| [Kumar et al., RMA, 2021](https://arxiv.org/abs/2107.04034) | Privileged base policy и adaptation module оценивают скрытые внешние факторы по истории | History/latent estimator для payload/friction/delay и частичной наблюдаемости stair task |
| [Rudin et al., Parkour in the Wild, 2025](https://arxiv.org/abs/2505.11164) | Terrain experts → DAgger-style distillation → RL fine-tuning; единая policy с нуля склонна к mode collapse | Сначала доказать up/down/rough experts, затем объединять; при fine-tune сохранять replay старых terrain |
...
| [Li et al., Scaling Rough Terrain Locomotion with Automatic Curriculum RL, 2026 preprint](https://arxiv.org/abs/2601.17428) | Авторы адаптируют распределение задач по измеренному прогрессу обучения на разных типах рельефа | Мотивирует явный учёт исходов по семейству и фазе; наш фиксированный reset pilot не воспроизводит их алгоритм и требует парного контроля |
