...
Снимок на **23 сентября 2026, 18:41 МСК**. Текущий интерфейс — **57 входов → 16 действий**, как у референса. Расширение входов пользователь отклонил; предложение stair-v4 остаётся нереализованным архивом. **Принятой rough/stair политики пока нет.**
...
| RTX4070Ti12GB | Отдельный ПК пользователя | Не квалифицирован в этом проекте; выполненных запусков здесь не заявляем |

Базовый стек: robot_lab/IsaacLab v2.3.2, IsaacSim5.1, RSL-RL3.1.2. Локальный runtime гибридный, бинарные пакеты из `D:\isaacsim51`; серверный — pinned Docker с узким URDF2.4.30 adapter. Квалификация headless B2W не означает поддержку серверного GUI/RT/cameras. Vendor неизменен.
