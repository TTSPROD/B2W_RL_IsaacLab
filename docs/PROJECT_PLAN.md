...
План пересмотрен 17 сентября 2026 по выполненным тестам. [Отчёт о результатах](TRAINING_PROGRESS.md) и [последние результаты](results/2026-09-17-yaw-reward-final.json) фиксируют состояние на дату записи; текущий прогресс хранится в локальном job.json.

## Подтверждённое состояние

Актуальная серия — seeds 45/46/47; [датированный статус](results/2026-09-17-flat-qualification-status.json)
и [протокол](FLAT_QUALIFICATION.md). Физическая диагностика готовых политик пройдена,
приёмочные результаты новой серии ещё не получены.

| Направление | Полученный результат | Что ещё требуется |
|---|---|---|
| Локальные вычисления | Windows 11 / RTX 4070 Ti: GPU smoke 10 000 шагов, PPO/resume, длительный pilot и двойной benchmark 4096 пройдены | Длительный профиль 2×4096 также завершён; rough/stairs и GUI отдельно |
...
| Абляция yaw-награды | Оба single-policy gates пройдены на трёх наборах; control 299/300 без отказа, yaw2x 300/300; yaw2x улучшил yaw RMS на 32–48% | По заранее заданному правилу повторить weight 1,5 / mix 0,25 на независимых seeds; [протокол](FLAT_QUALIFICATION.md) |
| Rough / Stairs / SDK | Реализация и приёмка не выполнены | Переход только после соответствующих gates ниже |

Текущий режим Flat — два независимых процесса на одной RTX 4070 Ti, по 4096 сред,
без камер. Измеренная суммарная скорость 63 908 transitions/s против 38 195 при
2×2048 (+67,3%); пик VRAM 9583 MiB, минимальный запас 21,98%, максимум 65 °C.
Это выигрыш throughput в измеренных окнах, а не доказанное ускорение сходимости.
...
прошли single-policy gates. Идёт [контролируемая квалификация](FLAT_QUALIFICATION.md). Seed 42 показал 96/100 эпизодов без отказа; все четыре
...
Текущая серия исследовательская: seed 42 остался на 2048 средах и имел resume
...
6. Запущена очередь [Flat qualification](FLAT_QUALIFICATION.md). Nominal reference
   regression на 20260919 точно совпал с прежним отчётом; bounded physical
   diagnostics control/yaw2x/reference на 20260920 дали 100/100 без отказа
   и tracking pass у каждого. Smoke без resume пройден. Обучаются seeds 45/46,
   затем автоматически следует 47. Все три запуска с нуля: одинаковые 4096 сред, конфигурация/reset schedule, 2500 updates
   × 4096 × 24 = 245 760 000 transitions/seed; сначала пара 45/46, затем 47.
   Это проверка выбранной конфигурации с нуля, не точная реплика прежней
   upstream→mix→continuation траектории. Бюджет фиксирован, автопродления нет.
7. До запуска зафиксированы acceptance evaluation seeds: nominal 20261001,
   bounded_v1 20261002, по 100 эпизодов каждого профиля для каждого training
   seed и reference. Bounded_v1: friction 0,6–1,0 / 0,5–0,8 с dynamic≤static;
   mass/inertia и Kp/Kd ×0,9–1,1. COM/noise/latency/pushes исключены.
   Фактические свойства подтверждаются readback и стабильным digest.
   Каждый seed/profile проходит gate отдельно. Наборы 20260917/18/19 уже
   диагностические; 20260920 также не final hold-out. До всех проверок
   Flat gate открыт; автоматического перехода к Rough нет.
...
- P0: завершить зафиксированные 2500 updates каждого seed 45/46/47; проверить checkpoints/optimizer/TensorBoard, source hashes и export parity.
- P0: оценить все три финальных model_2499 и reference на nominal 20261001 / bounded_v1 20261002. Каждый seed обязан отдельно пройти оба профиля; без автоматического продления бюджета.
- P1: по результатам принять или отклонить Flat-конфигурацию. При провале разобрать сценарии, bias и контакты; новые гипотезы фиксировать отдельным экспериментом. До приёмки трёх seeds Flat gate открыт.
