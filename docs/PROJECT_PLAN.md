...
План пересмотрен 17 сентября 2026 по выполненным тестам. [Отчёт о результатах](TRAINING_PROGRESS.md) и [снимок доказательств](results/2026-09-17.json) фиксируют состояние на дату записи; текущий прогресс хранится в локальном job.json.

## Подтверждённое состояние

| Направление | Полученный результат | Что ещё требуется |
|---|---|---|
| Локальные вычисления | Windows 11 / RTX 4070 Ti: GPU smoke 10 000 шагов, PPO/resume, длительный pilot и двойной benchmark 4096 пройдены | Длительный профиль 2×4096 завершится вместе с текущими runs; rough/stairs и GUI отдельно |
...
| Flat seeds 43/44 | Продолжение двух runs по 4096 сред после успешного benchmark | Завершение, экспорт и одинаковые evaluations; качество ещё не измерено |
...
Flat: завершить текущие seeds 43/44 и измерить стойку, движение, повороты и
остановку одним evaluator. Seed 42 показал 96/100 эпизодов без отказа; все четыре
...
1. Закончить активные runs. У seed 43 осталось 2089 updates с момента перезапуска
   от model_610, у seed 44 — 2189 от model_410. Учитывать переходы всех сохранённых
   сегментов, включая успешный benchmark: 245 710 848 transitions/seed.
   Это на 49 152 (ровно 0,02%) меньше исходной цели из-за округления размера batch.
2. После завершения обоих процессов автоматически выполнить checkpoint/optimizer/
   TensorBoard validation, экспорт и 100 эпизодов по 20 s. Сохранить каждый отказ,
   RMS по семействам, p95/max и интервал доли эпизодов без отказа.
3. Если yaw остаётся слабым, сначала записать распределение training-команд,
   tracking/reward components и контактировавшие звенья. Проверить гипотезу о
   недостатке чистых поворотов с vx=vy=0. Затем провести одну абляцию распределения
   команд при равном бюджете; rewards и физику одновременно не менять.
4. После выбора конфигурации провести контролируемый baseline по трём training
   seeds: одинаковые 4096 сред, параметры, reset schedule и бюджет. Для нового
   запуска с нуля 2500 updates × 4096 × 24 = 245 760 000 transitions.
   Seeds 43/44 текущей смешанной серии не заменяют эту проверку.
5. Перед приёмкой зафиксировать новые evaluation seeds и отдельный набор
   ограниченных физических randomizations. Набор eval seed 20260917 уже использован
   для диагностики и не является нетронутым финальным hold-out после подбора.
   Повторить reference тем же evaluator. Перейти к Rough только после Flat gate.

Reward tracking скорости/yaw и penalties за мощность, torque, резкие действия,
bad contacts, ориентацию и limits сохраняются upstream до отдельной обоснованной
абляции. Для колёс отдельно проверять rolling consistency и пробуксовку.
...
- P0: завершить текущие runs 43/44 на 4096, собрать длительную телеметрию, автоматически экспортировать и оценить оба checkpoint.
- P0: после сравнения с seed 42 и reference диагностировать yaw; проверить одну гипотезу за эксперимент при равном числе transitions.
- P1: зафиксировать итоговую Flat-конфигурацию и выполнить три сопоставимых seeds с новым evaluation hold-out и ограниченными randomizations. До этого Flat gate открыт.
