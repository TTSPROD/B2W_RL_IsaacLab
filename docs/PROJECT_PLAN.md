...
1. Разбор сохранённых трасс выполнен: [offline-анализ yaw/zero](analysis/operating57_19999_yaw_stop/README.md).
   При yaw−0.7 нарушения vy сохраняются после 20 s в 31/32 эпизодах; из 70 отказов
   нуля 66 вызваны только yaw. Причина трёх RR hip violations пока не установлена:
   сохранённая telemetry не содержит joint/action time series. Проверены hashes,
   body-frame измерения и неизменность всех 1152 оценок; нового screen не было.
...
4. Подготовлен [протокол одного ограниченного A/B-эксперимента](experiments/19999_yaw_stop_v1.md)
   на 19999: гипотеза изменения распределения команд, controls, seeds, бюджет,
   stop/regression criteria. Причинная гипотеза ещё не проверена; launcher и
   candidate evaluator требуют реализации. Перед resume синхронизировать adaptive
   LR с сохранённым Adam LR checkpoint и проверить это на серверном runtime.
   Измеренный sim2real gap определяет изменения actuator model и DR. Новое обучение
   и server job требуют отдельного решения.
