"""Two paired development continuations, contact weight -3 versus -6; fixed budget."""
import json
import math
import os
import shutil
...
NAME='flat_contact6_dev50_51_20260919'
...
DIAG=ROOT/'docs/results/2026-09-19-contact6-next-decision.json'
...
WEIGHTS={'control':-3.,'contact6':-6.}
SOURCES=(*BASE_SOURCES,'run_contact6_ablation.py','yaw_trace.py','b2w_height_rewards.py')
...
def training_spec(seed,arm,previous,smoke=False):
    ...
    return {'arm':arm,'seed':seed,'num_envs':16 if smoke else 4096,
            ...
            'expected_manifest':{'seed':seed,'num_steps_per_env':24,'pure_yaw_fraction':.25,
                                 ...
                                 'effective_base_height_weight':0.,'effective_base_height_form':'disabled',
...
def compare(results):
    ...
    variant=all(all(passes[f'seed{s}_contact6'].values()) for s in (50,51))
    c=sum(failures[f'seed{s}_control'] for s in (50,51))
    v=sum(failures[f'seed{s}_contact6'] for s in (50,51))
    selected=None
    if not ref: decision='reference_failed_investigate'
    elif control: selected='control';decision='prefer_unchanged_control'
    elif variant and v<c: selected='contact6';decision='contact6_candidate_for_fresh_replication'
    ...
    return {'decision':decision,'selected_for_future_replication':selected,'per_arm_profile_pass':passes,
            'failures_control':c,'failures_contact6':v,'release_accepted':False,
...
def verify_pair_configs(stage):
    ...
    for r in stage['runs']:
        ...
        cfg=yaml.load(path.read_text(encoding='utf-8'),Loader=yaml.BaseLoader)
        if float(cfg['rewards']['undesired_contacts']['weight'])!=WEIGHTS[r['arm']]:
            raise RuntimeError('Wrong contact weight')
        for name in ('base_height_l2','base_height_lower_l1'):
            if cfg['rewards'].get(name) not in (None,'null','None',''):
                raise RuntimeError('Height term must remain disabled')
        cfg['rewards']['undesired_contacts']['weight']='normalized'
        cfg.pop('log_dir',None)
        configs[r['arm']]=cfg
    if set(configs)!=set(WEIGHTS) or configs['control']!=configs['contact6']:
        raise RuntimeError('Unexpected paired environment difference')
    return {'only_env_differences':['log_dir','rewards.undesired_contacts.weight'],'contact_weights':WEIGHTS,'height_terms':'disabled'}
...
def main():
    ...
    if diagnosis.get('next_experiment')!='contact_weight_minus3_vs_minus6_height_disabled':
        raise RuntimeError('Completed diagnosis must select this registered hypothesis')
    if sha256(ROOT/diagnosis['diagnosis_report'])!=diagnosis['diagnosis_report_sha256'] or sha256(ROOT/diagnosis['source_job'])!=diagnosis['source_job_sha256']:
        raise RuntimeError('Completed result provenance changed')
    if sha256(TRACE_JOB)!=diagnosis['trace_job_sha256']:
        raise RuntimeError('Historical trace job changed')
    import psutil
    ...
    protocol={'seeds':[50,51],'seed_selection':'Known failing development seeds; not independent acceptance',
              ...
              'new_training_transitions':4*1500*4096*24,'execution':'control/contact6 pair for50, then pair51',
    ...
    shutil.copyfile(ROOT/'docs/CONTACT6_ABLATION.md',OUT/'protocol.md')
    ...
    def frozen():
        ...
        if sha256(DIAG)!=job['diagnosis_sha256'] or sha256(ROOT/diagnosis['diagnosis_report'])!=diagnosis['diagnosis_report_sha256']:raise RuntimeError('Decision/diagnosis changed')
    save();paired.OUT=OUT
    try:
