...
$outDir = Join-Path $root 'logs/stair_stop_pulses_20260923'
...
$profiles = @(
    [pscustomobject]@{ Label = 'rough55_stop_pulse10_v3open128'; Speed = '-0.10'; Steps = '10' },
    [pscustomobject]@{ Label = 'rough55_stop_pulse15_v3open128'; Speed = '-0.15'; Steps = '20' }
)
$jobs = @()
...
foreach ($profile in $profiles) {
    ...
    $cli = @(
        ...
        '--brake-distance', '1.2', '--brake-min-speed', '0.25',
        '--stop-pulse-speed', $profile.Speed, '--stop-pulse-steps', $profile.Steps
    )
