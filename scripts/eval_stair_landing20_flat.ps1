...
foreach ($seed in @(54, 55)) {
    ...
    if ((Test-Path $stdout) -or (Test-Path $stderr)) { throw "Output exists: $label flat $EvalSeed" }
    $cli = @(
    ...
    Write-Output "START $label flat=$EvalSeed PID=$($process.Id) actor=57"
}
...
foreach ($job in $jobs) {
    ...
    if(-not $exitOk -or -not $eval){throw "Evaluation failed: $($job.Label)"}
}
