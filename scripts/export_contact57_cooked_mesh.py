...
def extract(env, robot, compiled):
    ...
    for shape in geometry()['shapes']:
        ...
        get_physx_cooking_interface().request_convex_collision_representation(
            stage_id=stage_id, collision_prim_id=PhysicsSchemaTools.sdfPathToInt(shape['geometry_path']),
    ...
    save_json(BASE/'isaac_cooked_mesh.json',{'results':results,'source_sha256':sha256(__file__),
        'harness_sha256':sha256(BASE/'generated_export_cooked_mesh.py'),
...
if __name__ == '__main__':
    assert not (BASE/'isaac_cooked_mesh.json').exists()
    code = (BASE/'generated_export_geometry.py').read_text(encoding='utf-8')
    code = code.replace('from export_contact57_geometry import extract','from export_contact57_cooked_mesh import extract')
    code = code.replace('contact57_20260925/isaac_geometry.json','contact57_20260925/isaac_cooked_mesh.json')
    generated = BASE/'generated_export_cooked_mesh.py'
