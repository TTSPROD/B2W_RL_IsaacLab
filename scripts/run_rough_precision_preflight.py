"""Discard-only precision-reward fixture and exact own 2+2 PPO/resume smoke."""
...
PROTOCOL = ROOT/'docs/ROUGH_PRECISION_TRACKING.md'
SOURCES = tuple(dict.fromkeys((*BASE_SOURCES, 'rough_route_commands.py',
    'rough_precision_tracking.py', 'run_rough_route_correction.py',
    'run_reference_transfer.py', 'run_rough_precision_preflight.py',
    'run_rough_precision_training.py')))
...
def frozen_inputs():
    ...
    paths += [PROTOCOL, BASELINE, ROUTE_PREFLIGHT, ROUTE_ORIGINAL, ROUTE_FINAL, ANCHOR]
...
def verify_precision_run(path, smoke):
    """Compare complete effective env to route baseline, allowing only two std fields."""
    import yaml
    ...
    if (manifest.get('status') != 'completed' or not manifest.get('rough_transfer')
            or not manifest.get('rough_precision_tracking') or not manifest.get('rough_route_commands')
    ...
    if before != normalized:
        ...
        raise ValueError('Unregistered MDP change beyond tracking std: '+', '.join(differing))
    ...
    if smoke:
        for key in ('precision_tracking_fixture', 'route_command_fixture', 'tilt_termination_fixture'):
            if manifest.get(key, {}).get('passed') is not True:
                raise ValueError('Native fixture missing or failed: '+key)
    return dict(only_reward_changes=changed, old_std=.5, new_std=.25,
                weights={'linear': 3., 'yaw': 1.5}, scope='all Flat and Rough environments',
...
def main():
    ...
    name = f'rough_precision_preflight_20260920_{args.attempt}'
    ...
    job = dict(status='running', started_utc=utc_now(), supervisor_pid=os.getpid(), stages=[],
               route_commands=True, precision_tracking=True, seed=seed,
               scope='Native precision/route/tilt fixtures and 2+2 PPO/resume; discard all weights',
    ...
    try:
        ...
        for label, start in (('train', 0), ('resume', 2)):
            ...
            command = [PYTHON, '-B', '-u', 'scripts/train_b2w.py', '--rough_transfer',
                '--rough_tilt_termination', '--rough_route_commands', '--rough_precision_tracking',
            ...
            stage['precision_validation'] = verify_precision_run(ROOT/previous['training_manifest'], smoke=True)
            save()
        job['status'] = 'passed'
        return 0
