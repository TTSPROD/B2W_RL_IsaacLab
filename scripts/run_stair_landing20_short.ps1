...
$parents = @(
    [pscustomobject]@{
        Seed = 54
        Directory = '2026-09-23_11-02-18_stair_basevel_control57_2048_20260923_seed54_full'
        Sha = '49c50eabfc3d9ce640dc3cc46fb15f45fb8f071351e53a768afc2a406b474801'
    },
    [pscustomobject]@{
        Seed = 55
        Directory = '2026-09-23_11-04-25_stair_basevel_control57_2048_20260923_seed55_full'
        Sha = 'f5d7403f98853374b3cdaf6dc329461b29f0f0658e308c0d6a47b890c2371ff2'
    }
)
...
foreach ($item in $parents) {
    ...
    if (-not (Test-Path -LiteralPath $parent -PathType Leaf)) { throw "Missing parent: $parent" }
    if ((Get-FileHash -Algorithm SHA256 -LiteralPath $parent).Hash.ToLowerInvariant() -ne $item.Sha) {
        throw "Parent hash mismatch for seed $($item.Seed)"
    }
    $suffix = if ($Tag) { "${Mode}_${Tag}" } else { $Mode }
    $label = "stair_landing20_short_2048_20260923_seed$($item.Seed)_$suffix"
    ...
    $cli = @(
        ...
        '--landing-tiles-fraction', '0.2', '--landing-start-probability', '1.0',
...
foreach ($job in $jobs) {
    ...
    Write-Output "DONE $($job.Label) exit=$exitCode loaded=$loaded trained=$trained actor57=$task57 probes=$probes"
    if (-not $exitOk -or -not $loaded -or -not $trained -or -not $task57 -or -not $probes) {
