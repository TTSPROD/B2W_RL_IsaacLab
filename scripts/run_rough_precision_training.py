"""Two fresh Flat54-derived seeds; narrower tracking kernels and frozen quality gates."""
...
from run_rough_precision_preflight import (
    SOURCES, PROTOCOL, BASELINE, ROUTE_FINAL, frozen_inputs, require_finite, verify_precision_run,
)

NAME = 'rough_precision_training_20260920'
SEEDS = (61, 62)
BUDGET = (50, 100, 200)
...
def validate_preflight(path):
    ...
    if (preflight.get('status') != 'passed' or not preflight.get('precision_tracking')
            or not preflight.get('route_commands') or preflight.get('num_envs') != 64
    ...
    for index, stage in enumerate(preflight['stages']):
        ...
        if verify_precision_run(ROOT/current['training_manifest'], smoke=True) != stage['precision_validation']:
...
def main():
    ...
    job = dict(status='starting', started_utc=utc_now(), supervisor_pid=os.getpid(), stages=[], milestones=[],
        ...
        correction='Only linear/yaw exponential tracking std .5 to .25, unchanged weights 3/1.5, on Flat and Rough',
        historical_comparison='Route59/60 are historical comparators, not paired causal controls',
    ...
    try:
        for stage_index, updates in enumerate(BUDGET):
            ...
            for seed in SEEDS:
                ...
                stage['precision_validation'] = verify_precision_run(ROOT/result['training_manifest'], smoke=False)
