"""Two paired development continuations, disabled versus one-sided linear height shortfall; fixed budget."""
import json
import math
import os
import shutil
...
NAME='flat_height_floor_dev50_51_20260919'
...
DIAG=ROOT/'docs/results/2026-09-19-height-next-decision.json'
TRACE_JOB=ROOT/'logs/diagnostics/height_yaw_diagnostics_20260919/job.json'
PYTHON=str(ROOT/'.venv/Scripts/python.exe')
EVALUATIONS={'nominal':20261201,'bounded_v1':20261202}
WEIGHTS={'control':0.,'floorl1':-10.}
SOURCES=(*BASE_SOURCES,'run_height_floor_ablation.py','yaw_trace.py','b2w_height_rewards.py')
...
def training_spec(seed,arm,previous,smoke=False):
    ...
    return {'arm':arm,'seed':seed,'num_envs':16 if smoke else 4096,
            ...
            'extra_args':['--pure_yaw_fraction','.25','--yaw_tracking_weight','1.5',
                          '--undesired_contact_weight','-3']+(['--base_height_weight',str(WEIGHTS[arm]),'--base_height_form','lower_l1'] if WEIGHTS[arm] else []),
            'expected_manifest':{'seed':seed,'num_steps_per_env':24,'pure_yaw_fraction':.25,
                                 ...
                                 'effective_base_height_form':'lower_l1' if WEIGHTS[arm] else 'disabled',
...
def compare(results):
    ...
    variant=all(all(passes[f'seed{s}_floorl1'].values()) for s in (50,51))
    c=sum(failures[f'seed{s}_control'] for s in (50,51))
    v=sum(failures[f'seed{s}_floorl1'] for s in (50,51))
    selected=None
    if not ref: decision='reference_failed_investigate'
    elif control: selected='control';decision='prefer_unchanged_control'
    elif variant and v<c: selected='floorl1';decision='floorl1_candidate_for_fresh_replication'
    ...
    return {'decision':decision,'selected_for_future_replication':selected,'per_arm_profile_pass':passes,
            'failures_control':c,'failures_floorl1':v,'release_accepted':False,
...
def verify_pair_configs(stage):
    ...
    control=configs['control'];variant=configs['floorl1']
    term=variant['rewards'].pop('base_height_lower_l1')
    if float(term['weight'])!=-10. or float(term['params']['target_height'])!=.60:
        raise RuntimeError('Wrong lower-L1 height term')
    if term['func']!='b2w_height_rewards:base_height_lower_l1' or set(term['params'])!={'target_height'}:
        raise RuntimeError('Unexpected lower-L1 function/parameters')
    if control['rewards'].get('base_height_lower_l1') not in (None,'null','None',''):
        raise RuntimeError('Control lower-L1 term must be disabled')
    control['rewards'].pop('base_height_lower_l1',None)
    for cfg in configs.values():
        if cfg['rewards']['base_height_l2'] not in ('null','None',''):
            raise RuntimeError('L2 term must remain disabled in both arms')
        cfg.pop('log_dir',None)
    if control!=variant:raise RuntimeError('Additional environment configuration difference')
    return {'only_env_differences':['log_dir','rewards.base_height_lower_l1'],
            'height_form':'lower_l1','height_weight':-10.,'height_target_m':.60}
...
def main():
    ...
    if len(traces['stages'])!=8 or not all(s.get('original_results_exactly_reproduced') for s in traces['stages'].values()):
        raise RuntimeError('Passive instrumentation changed evaluation')
    if diagnosis.get('next_experiment')!='contact3x_height_disabled_vs_lower_l1_weight_minus10_target060':
        raise RuntimeError('Completed diagnosis must select this registered hypothesis')
    if sha256(ROOT/diagnosis['diagnosis_report'])!=diagnosis['diagnosis_report_sha256']:
        raise RuntimeError('Underlying measured diagnosis changed')
    if diagnosis['trace_job_sha256']!=sha256(TRACE_JOB):
        raise RuntimeError('Diagnosis trace provenance mismatch')
    import psutil
    for proc in psutil.process_iter(['cmdline']):
        cmd=proc.info['cmdline'] or []
        if any('train_b2w_desktop.py' in x for x in cmd) and any(str(ROOT).lower() in x.lower() for x in cmd):
    ...
    protocol={'seeds':[50,51],'seed_selection':'Known failing development seeds; not independent acceptance',
              ...
              'num_envs':4096,'rollout_steps':24,'height_form':'lower_l1','height_weights':WEIGHTS,'height_target_m':.60,'contact_weight':-3.,'pure_yaw_fraction':.25,'yaw_tracking_weight':1.5,
              'same_parent_model_optimizer_per_seed':True,'simulator_rng_reset_at_same_boundary':2500,
              'new_training_transitions':4*1500*4096*24,'execution':'control/floorl1 pair for50, then pair51',
    ...
    shutil.copyfile(ROOT/'docs/HEIGHT_FLOOR_ABLATION.md',OUT/'protocol.md')
    shutil.copyfile(DIAG,OUT/'decision.json')
    shutil.copyfile(ROOT/diagnosis['diagnosis_report'],OUT/'diagnosis.json')
    ...
    def frozen():
        ...
        if sha256(DIAG)!=job['diagnosis_sha256'] or sha256(ROOT/diagnosis['diagnosis_report'])!=diagnosis['diagnosis_report_sha256']:raise RuntimeError('Diagnosis/decision changed')
    save();paired.OUT=OUT
    try:
