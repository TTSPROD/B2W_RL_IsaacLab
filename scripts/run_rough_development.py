"""Two fresh Rough seeds, frozen50+100+200 schedule and mandatory milestone evaluation."""
...
NAME='rough_development_20260920';SEEDS=(57,58);BUDGET=(50,100,200)
...
def main():
    ...
    if not preflight.is_relative_to(ROOT) or pre['status']!='passed':raise ValueError('Passed project preflight required')
    ...
    job=dict(status='starting',started_utc=utc_now(),supervisor_pid=os.getpid(),stages=[],milestones=[],
             ...
             protocol='docs/ROUGH_DEVELOPMENT.md',protocol_sha256=sha256(ROOT/'docs/ROUGH_DEVELOPMENT.md'),
    ...
    job['source_sha256']['docs/ROUGH_DEVELOPMENT.md']=job['protocol_sha256']
    save=lambda:write_json(OUT/'job.json',job)
    for n in sources:shutil.copyfile(ROOT/'scripts'/n,OUT/'source'/n)
    shutil.copyfile(ROOT/'docs/ROUGH_DEVELOPMENT.md',OUT/'source/ROUGH_DEVELOPMENT.md')
    ...
    try:
        ...
        for stage_index,updates in enumerate(BUDGET):
            ...
            for seed in SEEDS:
                ...
                cmd=[PYTHON,'-B','-u','scripts/train_b2w_desktop.py','--rough_transfer','--headless','--device','cuda:0',
                     '--num_envs','4096','--seed',str(seed),'--max_iterations',str(updates),'--run_name',label,
        ...
        job['finished_utc']=utc_now();save();write_json(ROOT/'docs/results/rough_development_20260920.json',job)
