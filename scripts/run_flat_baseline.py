...
def main():
    ...
    source_names=['train_b2w.py','b2w_runtime.py','check_policy_contract.py','replay_reference_b2w.py','check_stand_b2w.py','flat_evaluation.py','run_flat_baseline.py','benchmark_b2w.py','smoke_b2w.py']
    (output/'source').mkdir()
    job['source_sha256']={}
    ...
    try:
        for seed in args.seeds:
            ...
            command=[python,'-B','-u','scripts/train_b2w.py','--headless','--device','cuda:0','--num_envs',str(args.num_envs), '--seed',str(seed),'--max_iterations',str(args.iterations),'--run_name',suffix]
