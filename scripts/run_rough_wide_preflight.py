"""Discard-only wider training corridor fixtures and exact own 2+2 PPO/resume smoke."""
...
PROTOCOL = ROOT/'docs/ROUGH_WIDE_CORRIDOR.md'
SOURCES = tuple(dict.fromkeys((*BASE_SOURCES, 'rough_route_commands.py',
    ...
    'run_rough_corridor_training.py', 'run_rough_wide_preflight.py',
    'run_rough_wide_training.py')))
...
CORRIDOR_FINAL = ROOT/'docs/results/rough_corridor_training_20260920.json'
...
def frozen_inputs():
    ...
    paths += [PROTOCOL, BASELINE, ROUTE_PREFLIGHT, ROUTE_ORIGINAL, ROUTE_FINAL, PRECISION_FINAL, CORRIDOR_FINAL, TRACE, ANCHOR]
...
def verify_wide_run(path, smoke):
    """Compare full effective env, preserving precision/terminal and verifying the wider training tracker bounds."""
    import yaml
    ...
    if (manifest.get('status') != 'completed' or not manifest.get('rough_transfer')
            or not manifest.get('rough_wide_corridor') or not manifest.get('rough_wheel_corridor') or not manifest.get('rough_precision_tracking') or not manifest.get('rough_route_commands')
    ...
    if manifest['rough_curriculum'].get('corridor_bounds') != dict(x=[-.6,5.4], y=[-1.8,1.8]):
        raise ValueError('Wider training corridor bounds differ from registered values')
    ...
    if smoke:
        for key in ('precision_tracking_fixture', 'route_command_fixture', 'tilt_termination_fixture', 'wheel_corridor_wide_fixture'):
    ...
    return dict(reward_changes_from_historical_route=changed, old_std=.5, new_std=.25,
                precision_recipe_preserved=True, wheel_corridor_terminal=True, wide_corridor=True,
                training_bounds=dict(x=[-.6,5.4],y=[-1.8,1.8]), evaluation_bounds=dict(x=[-.6,5.4],y=[-.9,.9]),
                wheel_corridor_fixture=manifest.get('wheel_corridor_wide_fixture'),
...
def main():
    ...
    name = f'rough_wide_preflight_20260920_{args.attempt}'
    ...
    prior = read(CORRIDOR_FINAL)
    if prior.get('status') != 'stopped_quality_gate' or prior.get('stopped_at') != 150:
        raise ValueError('Completed failed150 corridor experiment required')
    trace = read(TRACE)
    ...
    job = dict(status='running', started_utc=utc_now(), supervisor_pid=os.getpid(), stages=[],
               route_commands=True, precision_tracking=True, wheel_corridor=True, wide_corridor=True, seed=seed,
               scope='Native wide corridor/precision/route/tilt fixtures and 2+2 PPO/resume; discard all weights',
    ...
    try:
        ...
        for label, start in (('train', 0), ('resume', 2)):
            suffix = name+'_'+label
            command = [PYTHON, '-B', '-u', 'scripts/train_b2w.py', '--rough_transfer',
                '--rough_tilt_termination', '--rough_route_commands', '--rough_precision_tracking', '--rough_wheel_corridor', '--rough_wide_corridor',
            ...
            stage['wide_validation'] = verify_wide_run(ROOT/previous['training_manifest'], smoke=True)
            save()
        job['status'] = 'passed'
        return 0
