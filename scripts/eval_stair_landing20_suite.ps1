$ErrorActionPreference = 'Stop'
$root = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$launcher = Join-Path $PSScriptRoot 'run_local.ps1'
$runRoot = Join-Path $root 'logs/rsl_rl/unitree_b2w_stair'
$outDir = Join-Path $root 'logs/stair_landing20_20260923'
$benchmarkDir = Join-Path $root 'logs/stair_benchmark'
New-Item -ItemType Directory -Force -Path $outDir | Out-Null
$jobs = @()

foreach ($seed in @(54, 55)) {
    $runLabel = "stair_landing20_short_2048_20260923_seed${seed}_full"
    $runs = @(Get-ChildItem -LiteralPath $runRoot -Directory | Where-Object { $_.Name.EndsWith("_$runLabel") })
    if ($runs.Count -ne 1) { throw "Expected one run for $runLabel, found $($runs.Count)" }
    $checkpoint = Join-Path $runs[0].FullName 'model_422.pt'
    $manifest = Get-Content -LiteralPath (Join-Path $runs[0].FullName 'stair_parent.json') -Raw | ConvertFrom-Json
    if ($manifest.actor_observation_dim -ne 57 -or $manifest.actor_base_lin_vel -or
        $manifest.landing_tiles_fraction -ne 0.2) { throw "Invalid landing20 manifest: $checkpoint" }
    $label = "landing20_s${seed}_v3open128"
    if (Get-ChildItem -LiteralPath $benchmarkDir -Filter "${label}_cycle_*.json" -ErrorAction SilentlyContinue) {
        throw "Suite output exists: $label"
    }
    $stdout=Join-Path $outDir "${label}.suite.stdout.log"; $stderr=Join-Path $outDir "${label}.suite.stderr.log"
    $cli=@(
        '-NoProfile','-ExecutionPolicy','Bypass','-File',('"'+$launcher+'"'),
        'scripts/eval_stair_suite.py','--checkpoint',('"'+$checkpoint+'"'),'--label',$label,
        '--config','configs/stair_eval_v3.json','--split','development','--num-envs','128','--horizon','900',
        '--brake-profile','--brake-distance','1.2','--brake-min-speed','0.25'
    )
    $process=Start-Process powershell.exe -ArgumentList $cli -WorkingDirectory $root -WindowStyle Hidden `
        -RedirectStandardOutput $stdout -RedirectStandardError $stderr -PassThru
    $jobs += [pscustomobject]@{Label=$label; Process=$process; Stdout=$stdout}
    Write-Output "START $label PID=$($process.Id) actor=57"
}

foreach($job in $jobs){
    $job.Process.WaitForExit(); $job.Process.Refresh()
    $exitCode=$job.Process.ExitCode; $exitOk=$null -eq $exitCode -or $exitCode -eq 0
    $files=@(Get-ChildItem -LiteralPath $benchmarkDir -Filter "$($job.Label)_cycle_*.json")
    if(-not $exitOk -or $files.Count -ne 6){throw "Suite failed: $($job.Label); rows=$($files.Count)"}
    $rows=@($files|%{Get-Content $_.FullName -Raw|ConvertFrom-Json})
    Write-Output "DONE $($job.Label) passage=$(($rows|measure passage_success -sum).Sum)/768 cycle=$(($rows|measure success -sum).Sum)/768 unsafe=$(($rows|measure unsafe -sum).Sum) stop_failed=$(($rows|measure stop_failed -sum).Sum) incomplete=$(($rows|measure incomplete -sum).Sum)"
}
