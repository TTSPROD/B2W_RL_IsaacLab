...
Run with this project's .venv Python. Each child preserves train_b2w.py defaults.
"""
...
def run_one(args, num_envs: int, output_dir: Path, nvidia_smi: str, report: dict, report_path: Path) -> dict:
    ...
    command = [sys.executable, "-B", str(PROJECT_ROOT / "scripts/train_b2w.py"), "--headless",
...
def main() -> int:
    ...
    report = {"schema_version": 1, "status": "running", "started_utc": utc_now(),
              ...
              "source_sha256": {name: sha256(PROJECT_ROOT / name) for name in
                                ("scripts/benchmark_b2w.py", "scripts/train_b2w.py", "scripts/b2w_runtime.py")}}
    write_json(report_path, report)
