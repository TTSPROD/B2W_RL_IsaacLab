"""Two fresh Flat54-derived seeds; wider training corridor and frozen narrow evaluation gates."""
...
from run_rough_wide_preflight import (
    SOURCES, PROTOCOL, BASELINE, ROUTE_FINAL, frozen_inputs, require_finite, verify_wide_run,
)

NAME = 'rough_wide_training_20260920'
SEEDS = (65, 66)
BUDGET = (50, 100, 200)
...
def validate_preflight(path):
    ...
    if (preflight.get('status') != 'passed' or not preflight.get('precision_tracking')
            or not preflight.get('wide_corridor') or not preflight.get('wheel_corridor') or not preflight.get('route_commands') or preflight.get('num_envs') != 64
    ...
    for index, stage in enumerate(preflight['stages']):
        ...
        if verify_wide_run(ROOT/current['training_manifest'], smoke=True) != stage['wide_validation']:
...
def main():
    ...
    job = dict(status='starting', started_utc=utc_now(), supervisor_pid=os.getpid(), stages=[], milestones=[],
        ...
        correction='Double only training corridor half-width .9 to1.8m; terminal and curriculum preserved; narrow evaluation unchanged',
        historical_comparison='Corridor63/64 and precision61/62 at150 are historical comparators, not paired causal controls',
        wide_corridor=True, training_corridor_y=[-1.8,1.8], evaluation_corridor_y=[-.9,.9],
    ...
    try:
        for stage_index, updates in enumerate(BUDGET):
            ...
            for seed in SEEDS:
                ...
                stage['wide_validation'] = verify_wide_run(ROOT/result['training_manifest'], smoke=False)
