...
Example smoke run (two PPO updates, not a policy-quality evaluation)::
    python scripts/train_b2w_desktop.py --headless --num_envs 16 --max_iterations 2 --run_name smoke
"""
    ...
    try:
        ...
        runtime = {
            ...
            "source_sha256": {
                ...
                for name in ("scripts/train_b2w_desktop.py", "scripts/b2w_runtime.py", "scripts/b2w_yaw_commands.py", "scripts/yaw_command_sampling.py", "scripts/b2w_height_rewards.py", "scripts/reference_transfer.py", "vendor/manifest.json")
            },
        }
        ...
        source_dir.mkdir()
        for name in ("train_b2w_desktop.py", "b2w_runtime.py", "b2w_yaw_commands.py", "yaw_command_sampling.py", "b2w_height_rewards.py", "reference_transfer.py"):
        ...
        if args.rough_transfer:
            ...
            safe_curriculum = curriculum_class(env, cap=args.rough_stage, state=restored, **curriculum_kwargs)
            ...
            if args.rough_wheel_corridor and args.num_envs == 64:
                if args.rough_wide_corridor:
                    from rough_wheel_corridor import validate_wide_corridor_fixture
                    manifest['wheel_corridor_wide_fixture'] = validate_wide_corridor_fixture(env)
                else:
