"""Corrective Rough experiment: one tilt terminal; reuse both verified critic-calibration parents."""
...
NAME='rough_tilt_correction_20260920';SEEDS=(57,58);BUDGET=(50,100,200)
...
def verify_config_change(before_dir, after_dir, smoke=False):
    import yaml
    before,after=[yaml.load((d/'params/env.yaml').read_text(encoding='utf-8'),Loader=yaml.BaseLoader) for d in (before_dir,after_dir)]
    expected={'func':'rough_tilt_termination:sustained_tilt','params':{},'time_out':'false'}
    if after['terminations'].get('rough_sustained_tilt')!=expected:
        raise ValueError('Missing or incorrect true tilt terminal')
    if before['terminations'].get('rough_sustained_tilt') not in (None,expected):
        raise ValueError('Unexpected previous tilt terminal')
    before['terminations']['rough_sustained_tilt']=expected
    for cfg in (before,after):
        cfg.pop('log_dir',None)
        if smoke:cfg.pop('seed',None)
    if before!=after:raise ValueError('Unregistered MDP change beyond tilt terminal')
    agents=[yaml.load((d/'params/agent.yaml').read_text(encoding='utf-8'),Loader=yaml.BaseLoader) for d in (before_dir,after_dir)]
    for key in ('algorithm','policy','obs_groups','num_steps_per_env','clip_actions'):
        if agents[0][key]!=agents[1][key]:raise ValueError('Unregistered PPO change: '+key)
    return dict(only_mdp_change='terminations.rough_sustained_tilt',ppo_preserved=True)
...
def main():
    ...
    if not preflight.is_relative_to(ROOT) or pre['status']!='passed' or not pre.get('tilt_termination'):raise ValueError('Passed project preflight required')
    for path,h in pre['source_sha256'].items():
        if sha256(ROOT/path)!=h:raise ValueError('Preflight source changed: '+path)
    old_preflight=read(ROOT/'docs/results/rough_r1_preflight_20260920_6.json')
    old_smoke=next(s for s in old_preflight['stages'] if s['name']=='train')
    new_smoke=next(s for s in pre['stages'] if s['name']=='train')
    preflight_config=verify_config_change((ROOT/old_smoke['validation']['training_manifest']).parent,
                                         (ROOT/new_smoke['validation']['training_manifest']).parent,smoke=True)
    for s in pre['stages']:
        if s['name'] in ('train','resume') and not s['tilt_fixture']['passed']:raise ValueError('Incomplete native tilt/reset fixture')
    ...
    prior_path=ROOT/'docs/results/rough_development_20260920.json';prior=read(prior_path)
    if prior['status']!='failed' or prior['active_milestone']!=150 or prior['active_seed']!=57:
        raise ValueError('Expected the disclosed failed Rough experiment')
    first=prior['milestones'][0]
    if first['cumulative_updates']!=50 or not first['all_gates_passed']:
        raise ValueError('Both calibration parents must have passed their complete Flat gates')
    initial={}
    for seed in SEEDS:
        old=first['training'][str(seed)]
        if sha256(ROOT/old['checkpoint'])!=old['checkpoint_sha256'] or sha256(ROOT/old['policy'])!=old['policy_sha256']:
            raise ValueError('Calibration parent changed')
        old_manifest=read(ROOT/old['training_manifest'])
        if old_manifest.get('rough_tilt_termination') or old_manifest['rough_stage']!=0 or old_manifest['seed']!=seed:
            raise ValueError('Unexpected calibration parent lineage')
        initial[seed]=verify_run(f'rough_development_20260920_s{seed}_stage0',4096,0,50,seed,True)
        properties={}
        for profile in FLAT_SEEDS:
            evidence=first['flat'][str(seed)][profile];value=read(ROOT/evidence['report'])
            if not evidence['passed'] or sha256(ROOT/evidence['report'])!=evidence['sha256']:
                raise ValueError('Calibration Flat gate changed')
            verify_evaluation(value,old['policy_sha256'],profile,value['cases'],properties,evaluation_seed=FLAT_SEEDS[profile])
            if not summary_pass(summarize_flat(value['results'])):raise ValueError('Parent Flat raw rows failed')
    # This is a new one-factor branch from both own model_49 parents, never a retry from stopped_115.
    for p in (ROOT/'logs/rsl_rl/unitree_b2w_rough').glob('*_'+NAME+'*/manifest.json'):
        raise ValueError('Corrective experiment already started; no automatic replacements')
    ...
    job=dict(status='starting',started_utc=utc_now(),supervisor_pid=os.getpid(),stages=[],milestones=[],
             ...
             max_training_transitions=2*300*4096*24,reused_calibration_updates=100,discarded_original_stage1_updates=66,
             cumulative_main_updates_including_prior_failure=766,prior_failed_job=str(prior_path.relative_to(ROOT)),prior_failed_job_sha256=sha256(prior_path),
             preflight_config=preflight_config,correction='Only add sustained tilt terminal; own critic/optimizer calibration parents retained',initial_parents=initial,anchor=str(ANCHOR.relative_to(ROOT)),anchor_sha256=sha256(ANCHOR),
             ...
             protocol='docs/ROUGH_TILT_CORRECTION.md',protocol_sha256=sha256(ROOT/'docs/ROUGH_TILT_CORRECTION.md'),
    ...
    job['source_sha256']['docs/ROUGH_TILT_CORRECTION.md']=job['protocol_sha256']
    save=lambda:write_json(OUT/'job.json',job)
    for n in sources:shutil.copyfile(ROOT/'scripts'/n,OUT/'source'/n)
    shutil.copyfile(ROOT/'docs/ROUGH_TILT_CORRECTION.md',OUT/'source/ROUGH_TILT_CORRECTION.md')
    cases={f'{f}/{l}/{p}':make_cases(f,l,p) for f in FAMILIES for l in range(3) for p in CASE_SEEDS}
    write_json(OUT/'frozen_cases.json',cases);job['rough_cases_sha256']=sha256(OUT/'frozen_cases.json')
    if job['rough_cases_sha256']!=prior['rough_cases_sha256']:raise ValueError('Frozen Rough cases changed')
    save()
    flat_physics={};rough_physics={};parents={};previous=dict(initial)
    try:
        ...
        for stage_index,updates in list(enumerate(BUDGET))[1:]:
            ...
            for seed in SEEDS:
                ...
                cmd=[PYTHON,'-B','-u','scripts/train_b2w_desktop.py','--rough_transfer','--headless','--device','cuda:0',
                     '--rough_tilt_termination','--num_envs','4096','--seed',str(seed),'--max_iterations',str(updates),'--run_name',label,
                ...
                stage['config_validation']=verify_config_change((ROOT/previous[seed]['training_manifest']).parent,(ROOT/result['training_manifest']).parent)
        ...
        job['finished_utc']=utc_now();save();write_json(ROOT/'docs/results/rough_tilt_correction_20260920.json',job)
